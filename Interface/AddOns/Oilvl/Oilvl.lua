...
local CNname2, _, _, _, _, _, _ = EJ_GetInstanceInfo(1296); -- Liberation of Undermine
local CNname3, _, _, _, _, _, _ = EJ_GetInstanceInfo(1302); -- Manaforge Omega
...
local DOSTATCN3 =
{
	...
	}, -- [7]
	{
		...
		19376, -- [3]
		19385, -- [4]
		"Tindral Sageswift, Seer of the Flame", -- [5]
	}, -- [8]
	{
		19359, -- [1]
		19368, -- [2]
		...
		for i = 1, 40 do
			...
			if ilvl == nil or ilvl == "" then
				...
				if IsInRaid() then
					...
					if CanInspect("raid"..i) then ORfbIlvl(i); return 0; end
				elseif IsInGroup(LE_PARTY_CATEGORY_INSTANCE) or IsInGroup(LE_PARTY_CATEGORY_HOME) then
					if i == 1 then
						...
						if CanInspect("party"..(i-1)) then ORfbIlvl(i); return 0; end
					end
				else
					ORfbIlvl(1); return 0;
				end
...
function OilvlRPDTimeCheck()
	if not UnitAffectingCombat("player") and OilvlTooltip:IsShown() then
		if GetMouseFoci()[1] == rpdframe and rpdframesw then
			OilvlSetCA();
			rpdframesw = false;
		end
	end
...
function oilvlcheckrange()
	if not UnitAffectingCombat("player") and oilvlframesw then
		...
		if IsInRaid() or IsInGroup(LE_PARTY_CATEGORY_INSTANCE) or IsInGroup(LE_PARTY_CATEGORY_HOME) then
			...
			ONumSHAMAN:Show(); ONumWARLOCK:Show(); ONumWARRIOR:Show(); ONumDEMONHUNTER:Show(); ONumEVOKER:Show();
			...
			rnum = GetNumGroupMembers();
			local cnum = {};
			for j = 1, 13 do cnum[j]=0 end
			if IsInRaid() then
				for i = 1, rnum do
					local _, _, cclass = UnitClass("raid"..i);
					for j = 1, 13 do if cclass == j then cnum[j] = cnum[j] + 1 end	end
				end
			else
				for i = 1, rnum do
			...
			OilvlAIL_EVOKER:SetText(cnum[13])
			OZeth = cnum[13]+cnum[10]+cnum[4]+cnum[1]
			ODred = cnum[6]+cnum[12]+cnum[9]
			OMyst = cnum[11]+cnum[3]+cnum[8]
			OVene = cnum[2]+cnum[5]+cnum[7]
			ZethText:SetText(OZeth)
			DredText:SetText(ODred)
			MystText:SetText(OMyst )
			VeneText:SetText(OVene)			
		else
			...
			ONumSHAMAN:Hide(); ONumWARLOCK:Hide(); ONumWARRIOR:Hide(); ONumDEMONHUNTER:Hide(); ONumEVOKER:Hide();
...
function OilvlSetCA()
	...
	if not rpsw and UnitExists(ounit) and cfg.oilvlms then
		Omover2=1;
		ClearAchievementComparisonUnit();
...
-- Average Item Level
	...
	CreateTextureFontString("ONumTank",260,104,ORole["TANK"][1],ORole["TANK"][2],ORole["TANK"][3],ORole["TANK"][4],ORole["TANK"][5],"OilvlAIL_TANK",278,104)
	CreateTextureFontString("ONumDPS", 260,86,ORole["DAMAGER"][1],ORole["DAMAGER"][2],ORole["DAMAGER"][3],ORole["DAMAGER"][4],ORole["DAMAGER"][5],"OilvlAIL_DPS",278,86)
	CreateTextureFontString("ONumHeal",260,68,ORole["HEALER"][1],ORole["HEALER"][2],ORole["HEALER"][3],ORole["HEALER"][4],ORole["HEALER"][5],"OilvlAIL_HEAL",278,68)

	-- Zenith
	CreateTextureFontString("ONumEVOKER",10,104,OClassTexture["BASE"],OClassTexture["EVOKER"][1],OClassTexture["EVOKER"][2],OClassTexture["EVOKER"][3],OClassTexture["EVOKER"][4],"OilvlAIL_EVOKER",28,104)
	CreateTextureFontString("ONumMONK",60,104,OClassTexture["BASE"],OClassTexture["MONK"][1],OClassTexture["MONK"][2],OClassTexture["MONK"][3],OClassTexture["MONK"][4],"OilvlAIL_MONK",78,104)
	CreateTextureFontString("ONumROGUE",110,104,OClassTexture["BASE"],OClassTexture["ROGUE"][1],OClassTexture["ROGUE"][2],OClassTexture["ROGUE"][3],OClassTexture["ROGUE"][4],"OilvlAIL_ROGUE",128,104)
	CreateTextureFontString("ONumWARRIOR",160,104,OClassTexture["BASE"],OClassTexture["WARRIOR"][1],OClassTexture["WARRIOR"][2],OClassTexture["WARRIOR"][3],OClassTexture["WARRIOR"][4],"OilvlAIL_WARRIOR",178,104)
	OCreateFontString("ZethText",210,104)

	-- Dreadful
 	CreateTextureFontString("ONumDEATHKNIGHT",10,86,OClassTexture["BASE"],OClassTexture["DEATHKNIGHT"][1],OClassTexture["DEATHKNIGHT"][2],OClassTexture["DEATHKNIGHT"][3],OClassTexture["DEATHKNIGHT"][4],"OilvlAIL_DEATHKNIGHT",28,86)
	CreateTextureFontString("ONumDEMONHUNTER",60,86,OClassTexture["BASE"],OClassTexture["DEMONHUNTER"][1],OClassTexture["DEMONHUNTER"][2],OClassTexture["DEMONHUNTER"][3],OClassTexture["DEMONHUNTER"][4],"OilvlAIL_DEMONHUNTER",78,86)
	CreateTextureFontString("ONumWARLOCK",110,86,OClassTexture["BASE"],OClassTexture["WARLOCK"][1],OClassTexture["WARLOCK"][2],OClassTexture["WARLOCK"][3],OClassTexture["WARLOCK"][4],"OilvlAIL_WARLOCK",128,86)
	OCreateFontString("DredText",210,86)
	
	-- Mystic
	CreateTextureFontString("ONumDRUID",10,68,OClassTexture["BASE"],OClassTexture["DRUID"][1],OClassTexture["DRUID"][2],OClassTexture["DRUID"][3],OClassTexture["DRUID"][4],"OilvlAIL_DRUID",28,68)
	CreateTextureFontString("ONumMAGE",60,68,OClassTexture["BASE"],OClassTexture["MAGE"][1],OClassTexture["MAGE"][2],OClassTexture["MAGE"][3],OClassTexture["MAGE"][4],"OilvlAIL_MAGE",78,68)
	CreateTextureFontString("ONumHUNTER",110,68,OClassTexture["BASE"],OClassTexture["HUNTER"][1],OClassTexture["HUNTER"][2],OClassTexture["HUNTER"][3],OClassTexture["HUNTER"][4],"OilvlAIL_HUNTER",128,68)
	OCreateFontString("MystText",210,68)

	-- Venerated
	CreateTextureFontString("ONumPALADIN",10,50,OClassTexture["BASE"],OClassTexture["PALADIN"][1],OClassTexture["PALADIN"][2],OClassTexture["PALADIN"][3],OClassTexture["PALADIN"][4],"OilvlAIL_PALADIN",28,50)
	CreateTextureFontString("ONumPRIEST",60,50,OClassTexture["BASE"],OClassTexture["PRIEST"][1],OClassTexture["PRIEST"][2],OClassTexture["PRIEST"][3],OClassTexture["PRIEST"][4],"OilvlAIL_PRIEST",78,50)
	CreateTextureFontString("ONumSHAMAN",110,50,OClassTexture["BASE"],OClassTexture["SHAMAN"][1],OClassTexture["SHAMAN"][2],OClassTexture["SHAMAN"][3],OClassTexture["SHAMAN"][4],"OilvlAIL_SHAMAN",128,50)
	OCreateFontString("VeneText",210,50)
...
function oilvlgetraidstat()
	...
	cfg.adh = {}
	OilvlGetStatisticId("Dragonflight","Amirdrassil",cfg.adh,true)
end
...
function OGetRaidProgression2(RaidName, OSTAT, NumRaidBosses, Faction)
	...
	bigorp[CNname2] = Save_orp(CNname2, OSTATCN2, 8)
	bigorp[CNname3] = Save_orp(CNname3, OSTATCN3, 8)
	...
	SaveAOTCCE(RaidAchiv[CNname2],41298,41297)
	SaveAOTCCE(RaidAchiv[CNname3],41624,41625)
	...
	local function DrawOTooltip2()
		...
		otooltip2:SetCell(3,4,"|cffffffff"..CNname3,"LEFT",2)
			...
			otooltip2:SetAutoHideDelay(0.25, self, function()
				...
				if not otooltip6sw and otooltip6 then
					otooltip6:SetAutoHideDelay(0.25,self,function()
						--ClearAchievementComparisonUnit();
						if GetMouseFoci()[1] ~= otooltip6 then
							otooltip6:Hide()
...
function otooltip5func()
	...
	otooltip5:AddSeparator()
	-- Altok rendezése item level szerint (legmagasabbtól a legalacsonyabbig)
	table.sort(cfg.oilvlgears, function(a, b)
	...
	for m = 1,  #cfg.oilvlgears do
		line = otooltip5:AddLine()
			-- Név osztályszínnel, ha cache-ben elérhető
		local cname, crealm = cfg.oilvlgears[m][1], cfg.oilvlgears[m][2]
		local colorPrefix = "|cFFFFFFFF"
		if cfg.oilvlcache and #cfg.oilvlcache > 0 then
			for r = 1, #cfg.oilvlcache do
				local C = cfg.oilvlcache[r]
				if C and C.oname == cname and C.orealm == crealm then
					if C.oclass then
						local code = string.match(C.oclass, "^|c%x%x%x%x%x%x%x%x")
						if code then colorPrefix = code end
					end
					break
				end
			end
		end
		local coloredName = colorPrefix..cname.."-"..crealm.."|r"
		otooltip5:SetCell(line, 1, coloredName)
...
function otooltip6func()
	...
	if IsInRaid() or IsInGroup(LE_PARTY_CATEGORY_INSTANCE) or IsInGroup(LE_PARTY_CATEGORY_HOME) then
		...
		otooltip6:AddSeparator()
		line = otooltip6:AddLine()		
		otooltip6:SetCell(line, 1, L["Zenith"],"CENTER",2)
		otooltip6:SetCell(line, 3, OZeth)
		line = otooltip6:AddLine()
		otooltip6:SetCell(line, 1, L["Dreadful"],"CENTER",2)
		otooltip6:SetCell(line, 3, ODred)
 		line = otooltip6:AddLine()
		otooltip6:SetCell(line, 1, L["Mystic"],"CENTER",2)
		otooltip6:SetCell(line, 3, OMyst)
		line = otooltip6:AddLine()
		otooltip6:SetCell(line, 1, L["Venerated"],"CENTER",2)
		otooltip6:SetCell(line, 3, OVene)
		otooltip6:AddSeparator()
	end
	line = otooltip6:AddLine()
		...
		if cfg.oilvlms then
			if Omover2 == 1 then
				if UnitExists(rpunit) and CheckInteractDistance(rpunit, 1) and rpsw then
					...
					if cfg.raidmenuid == 2 then OGetRaidProgression2(CNname2, OSTATCN2, 8, UnitFactionGroup(rpunit)); end
					if cfg.raidmenuid == 3 then OGetRaidProgression2(CNname3, OSTATCN3, 8, UnitFactionGroup(rpunit)); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
			...
			elseif Omover2 == 2 then
				if UnitExists(rpunit) and CheckInteractDistance(rpunit, 1) and rpsw then
					...
					if cfg.raidmenuid == 2 then OGetRaidProgression3(CNname2, OSTATCN2, 8, UnitFactionGroup(rpunit)); end
					if cfg.raidmenuid == 3 then OGetRaidProgression3(CNname3, OSTATCN3, 8, UnitFactionGroup(rpunit)); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
				...
				if UnitExists("target") and CheckInteractDistance("target", 1)  and rpsw then
					...
					if cfg.raidmenuid == 2 then OGetRaidProgression(CNname2, OSTATCN2, 8); end
					if cfg.raidmenuid == 3 then OGetRaidProgression(CNname3, OSTATCN3, 8); end
				else
					--ClearAchievementComparisonUnit();
					rpsw=false;
					rpunit="";
...
function events:PLAYER_LOGIN(...)
	...
	if cfg.raidmenuid  == nil then cfg.raidmenuid = 2; end
	...
	hookScript(GameTooltip, "OnTooltipSetUnit", function()
	 	if not UnitAffectingCombat("player")  and cfg.oilvlms and UnitExists("target") and not IsInRaid() and not IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and not IsInGroup(LE_PARTY_CATEGORY_HOME) then			
	 		...
	 		if  oname == GetUnitName("target",""):gsub("%-.+", "") then
				GameTooltip:SetHeight(GameTooltip:GetHeight()+15);
				GameTooltip:AddLine(L["Item Level"]..": |r|cFF00FF00"..targetilvl);	
				print(targetilvl)
	 			--OMouseover();
	 		end
	 	end
...
function OilvlConfigFrame()
	...
	cfg.frame = CreateFrame("Frame", "OiLvLConfig", UIParent, "ButtonFrameTemplate")
	cfg.frame:SetWidth(480);
	cfg.frame:SetHeight(540);
	cfg.frame:SetFrameStrata("LOW");
	oilvlcfgbutton(cfg.frame);

	cfg.frame:SetMovable(true);
	cfg.frame:EnableMouse(true);
	cfg.frame:RegisterForDrag("LeftButton");
	cfg.frame:SetScript("OnDragStart", cfg.frame.StartMoving);
	--f:SetScript("OnDragStop", function() f:StopMovingOrSizing();  cfg.oilvlframeP, _, _, cfg.oilvlframeX, cfg.oilvlframeY = f:GetPoint() end);
	cfg.frame:SetScript("OnDragStop", function() cfg.frame:StopMovingOrSizing();  cfg.oilvlframeP, _, _texture, cfg.oilvlframeX, cfg.oilvlframeY = cfg.frame:GetPoint() end);

-- Set Title
	cfg.frame.text = cfg.frame.text or cfg.frame:CreateFontString("oilvlname","ARTWORK", "GameTooltipText");
	cfg.frame.text:SetAllPoints(true);
	cfg.frame.text:SetPoint("TOPLEFT",0,-6);
	cfg.frame.text:SetJustifyH("CENTER");
	cfg.frame.text:SetJustifyV("TOP");
	cfg.frame.text:SetTextColor(1,1,1,1);
	cfg.frame.text:SetFont("Fonts\\FRIZQT__.TTF",12,"")
	cfg.frame.text:SetText("O Item Level");


	local t = cfg.frame:CreateTexture(nil,"BACKGROUND")
	t:SetColorTexture(0.1,0.1,0.1,0.5)
	t:SetAllPoints(f)
	cfg.frame.texture = t
	cfg.frame:SetPoint("CENTER",0,200);
	...
	title:SetPoint("TOPLEFT",76,-36)
	title:SetText("O Item Level (OiLvL)") -- can get version from GetAddOnMetadata
...
--  oilvl scale
	...
	oscale:SetPoint("TOPLEFT",16,-120);

	local scaletitle = oscale:CreateFontString(nil,"ARTWORK","GameFontNormal")
	scaletitle:SetPoint("LEFT",oscale,"LEFT",0,45)
...
--  oilvl opacity
	...
	oalpha:SetPoint("TOPLEFT",246,-120);
...
function oilvlchecktiers()
	local zenith, dreadful, mystic, venerated = 0,0,0,0
	if IsInRaid() then
		rnum = GetNumGroupMembers();
		for i = 1, rnum do
			local _, _, cclass = UnitClass("raid"..i);	
			--			 Evoker			 Monk			 Rogue			Warrior
			if cclass == 13 or cclass == 10 or cclass == 4 or cclass == 1 then zenith = zenith + 1 end
			--			 DK				DH				Warlock
			if cclass == 6 or cclass == 12 or cclass == 9 then dreadful = dreadful + 1 end
			--			 Druid			 Hunter		   Mage
			if cclass == 11 or cclass == 3 or cclass == 8 then mystic = mystic + 1 end
			--			 Paladin		Priest		   Shaman
			if cclass == 2 or cclass == 5 or cclass == 7 then venerated = venerated + 1 end
		end
	end
	return zenith, dreadful, mystic, venerated;
end
	...
	if (cfg.oilvlbagilvl) then
		...
		if (message and message > 1) then
			...
			if (not container.ilvl) then
				...
				container.ilvl:SetPoint("TOPLEFT", 8, -22)
...
function oilvlShowBagItemLevel()
	...
	while (frame and frame.Update) do
		hooksecurefunc(frame, "Update", UpdateCombinedContainer)
		id = id + 1
...
function OLDoilvlShowBagItemLevel()
	local quality_color = {
...
SlashCmdList["OILVL_OIT"] = function(msg)
	local zenith, dreadful, mystic, venerated = oilvlchecktiers()
	print("Zenith (Evoker, Monk, Rogue, Warrior): "..zenith)
	print("Dreadful (Death Knight, Demon Hunter, Warlock): "..dreadful)
	print("Mystic (Druid, Hunter, Mage): "..mystic)
	print("Venerated (Paladin, Priest, Shaman): "..venerated)
end
